...
export function AiPreview() {
  ...
  const handleSaveRoadmapPdf = () => {
    ...
    const actionPlan = generatedRoadmap.actionPlan
      ?.map(
        (action) => `
          <li>
            <strong>${escapeHtml(action.phase || "Next step")}: ${escapeHtml(action.title || "")}</strong>
      ...
      <html>
        <head>
          ...
          <style>
            ...
            h1 { color: #111827; font-size: 26px; margin: 0 0 4px; }
            h2 { color: #374151; font-size: 17px; margin: 24px 0 8px; }
            ...
            .highlight { border-left: 4px solid #f97316; background: #fff7ed; }
            ...
            li { margin: 8px 0; }
            li p { color: #6b7280; font-size: 13px; margin: 2px 0 0; }
            .closing { background: #f5f3ff; font-style: italic; }
            .footer { color: #9ca3af; font-size: 11px; margin-top: 28px; }
          </style>
        </head>
        <body>
          <h1>Your Transformation Roadmap</h1>
          <p class="subtitle">
            ${generatedRoadmap.category ? `Focus: ${escapeHtml(generatedRoadmap.category)} · ` : ""}
            Generated by Transformation Builder on ${new Date().toLocaleDateString()}
          </p>

          ${
            generatedRoadmap.strategicInsight
              ? `<div class="section highlight"><h2>Strategic Insight</h2><div>${escapeHtml(generatedRoadmap.strategicInsight)}</div></div>`
              : ""
          }
          ...
          <div class="stats">
            <div class="section">
              ...
              <div class="stat-value">${escapeHtml(generatedRoadmap.preference || "Mixed")}</div>
            </div>
            <div class="section">
              <div class="stat-label">Destination</div>
              <div class="stat-value">${escapeHtml(generatedRoadmap.destination || "Purpose & Impact")}</div>
            </div>
          </div>

          ${actionPlan ? `<div class="section"><h2>Action Plan</h2><ul>${actionPlan}</ul></div>` : ""}
          ${keyMetrics ? `<div class="section"><h2>Success Metrics</h2><ul>${keyMetrics}</ul></div>` : ""}
          ${
            generatedRoadmap.motivationalClose
              ? `<div class="section closing">${escapeHtml(generatedRoadmap.motivationalClose)}</div>`
              : ""
          }
          <div class="footer">Use your browser's print dialog to select “Save as PDF” and choose your Desktop as the save location.</div>
          <script>
