...
export default function DiscoverPurposePage() {
  ...
  // ✅ FIX: Use refs to track current recording state for the speech recognition callback
  const isRecordingRef = useRef<number | null>(null);
  ...
  // ✅ FIXED: Helper function to check if module is locked
  const isModuleLocked = (moduleId: number): boolean => {
    ...
    // Modules 4-9 require TRANSFORMER or higher
    ...
    const userPlan = (user?.plan || "EXPLORER") as string;
    const userRank = planRank[userPlan] || 1;

    // Need TRANSFORMER (rank 2) or higher for modules 4-9
    return userRank < 2;
  };

  // Helper function to go to next module
  const goToNextModule = () => {
    ...
    if (currentIndex < modules.length - 1) {
      ...
      if (!isModuleLocked(nextModule.id)) {
        ...
        if (isRecording !== null && recognitionRef.current) {
        ...
        toast({
          ...
          description: `Module ${nextModule.id} requires the Transformer plan or higher.`,
          variant: "destructive"
        });
      }
    }
  ...
  // ✅ FIX: Set up speech recognition only once, use refs for current values
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onresult = (event: any) => {
        // ✅ FIX: Use refs instead of state to get current values
        const currentRecordingIndex = isRecordingRef.current;
        const currentModule = activeModuleRef.current;

        if (currentRecordingIndex === null) return;
        ...
        if (finalTranscript) {
          const key = `${currentModule.id}-${currentRecordingIndex}`;
          setAnswers(prev => ({
            ...prev,
  ...
  }, [toast]); // ✅ FIX: Remove activeModule.id and isRecording from dependencies
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        <div className="max-w-4xl mx-auto text-center mb-12">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-8 p-4 rounded-xl bg-muted/20 border border-border/50 max-w-3xl mx-auto">
            ...
            <a 
              href="https://www.amazon.com/Understanding-Your-Path-Discovering-purpose/dp/B0F6DG44HV" 
              target="_blank" 
            ...
            >
              <img 
                src={bookCover} 
                alt="Understanding Your Path by Alexander Ligons" 
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (isModuleLocked(module.id)) {
                            toast({
                              title: "Upgrade Required",
                              description: `Modules 4-9 require the Transformer plan or higher.`,
                              variant: "destructive"
                            });
                            return;
                          }
                ...
                <Card className="border-primary/10 shadow-lg bg-white/80 backdrop-blur-md">
                  ...
                  <CardContent className="p-8 space-y-8">
                        ...
                        <div key={index} className="space-y-3">
                          ...
                          <div className="relative group">
                            ...
                            <Button
                              size="icon"
                              variant="ghost"
                              className={`absolute right-2 top-2 transition-colors cursor-pointer ${isRecording === index ? 'text-red-500 bg-red-50 hover:bg-red-100' : 'text-muted-foreground hover:text-primary hover:bg-primary/5'}`}
                    ...
                    <div className="flex flex-col items-center pt-4 gap-4">
                      {/* Save and Summary buttons */}
                      <div className="flex justify-center gap-4 w-full">
                        <Button 
                          size="lg" 
                          className="shadow-lg shadow-primary/20 cursor-pointer hover:bg-primary/90 hover:scale-[1.02] active:scale-[0.98] active:shadow-md transition-all duration-150" 
                        ...
                        {activeModule.id === 9 && (
                          <Link href="/discover-purpose/summary">
                            <Button size="lg" variant="outline" className="border-primary/20 hover:bg-primary/5 gap-2 cursor-pointer">
                      ...
                      {/* Next Module Navigation Arrow */}
                      {hasNextModule() && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={goToNextModule}
                          className="flex flex-col items-center gap-1 h-auto py-3 px-6 cursor-pointer hover:bg-primary/5 hover:text-primary transition-all group"
                        >
                          <span className="text-sm text-muted-foreground group-hover:text-primary transition-colors">
                            Next Module
                          </span>
                          <ChevronDown className="w-5 h-5 text-muted-foreground group-hover:text-primary group-hover:translate-y-1 transition-all" />
                        </Button>
                      )}
                    </div>
                  </CardContent>
